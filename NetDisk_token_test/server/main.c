...
void* threadfunc(void* p)
{
	...
	while(1){
		...
		if(0==ret){
			//先看客户端是注册还是登录
			recvCycle(pcur->nd_sockfd,&dataLen,4);
			recvCycle(pcur->nd_sockfd,buf,dataLen);
			if(!strcmp(buf,"R")){
				reg(pcur->nd_sockfd);//负责注册的函数
			}
			if(!strcmp(buf,"L")){
				logon(pcur->nd_sockfd);//控制用户登录
			}
			if(!strcmp(buf,"gets")){//处理客户端的子线程
				client_gets(pcur->nd_sockfd);
			}
			free(pcur);
		}
		pcur=NULL;
